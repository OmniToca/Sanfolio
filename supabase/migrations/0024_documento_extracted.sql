-- Uložená pole dokladu po Guardar. AI sem nezapisuje (jen ai_drafts).
-- ai_get_cliente čte extracted, ať copilot umí „kdy končí pas“.

ALTER TABLE documentos
  ADD COLUMN IF NOT EXISTS extracted JSONB NOT NULL DEFAULT '{}'::jsonb;

COMMENT ON COLUMN documentos.extracted IS
  'Strukturovaná pole dokladu po uložení gestorem. AI jen navrhne do ai_drafts.';
...
COMMENT ON FUNCTION public.ai_get_cliente(UUID) IS
  'Snapshot + díry + uložené doklady pro copilot. Audit ai.read.cliente. Žádný save/send.';
