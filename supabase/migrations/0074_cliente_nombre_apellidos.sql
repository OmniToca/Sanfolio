-- Karta má jméno a příjmení zvlášť. open_carpeta dřív obě slila do `nombre`.
-- p_apellidos na konci, 7-arg CSV volání pořád sedí.

DROP FUNCTION IF EXISTS public.open_carpeta_compraventa(
  UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT
);

CREATE FUNCTION public.open_carpeta_compraventa(
  p_tenant_id UUID,
  p_nombre TEXT,
...
AS $$
DECLARE
  v_raw TEXT := btrim(coalesce(p_nombre, ''));
  v_given TEXT;
...
BEGIN
  ...
  IF v_apellidos IS NULL AND position(' ' IN v_raw) > 0 THEN
    v_given := split_part(v_raw, ' ', 1);
    v_apellidos := nullif(btrim(substr(v_raw, char_length(v_given) + 2)), '');
  ELSE
    v_given := v_raw;
  END IF;
  ...
  END IF;
  v_full := btrim(v_given || ' ' || coalesce(v_apellidos, ''));
  ...
  INSERT INTO clientes (
    tenant_id, kind, nombre, apellidos, email, tel, direccion, iban, locale, status
  ) VALUES (
    p_tenant_id,
    'persona',
    v_given,
  ...
  INSERT INTO expedientes (
    ...
    p_tenant_id, v_cliente, v_inmueble, 'compraventa', 'abierto', v_full
  )
...
COMMENT ON FUNCTION public.open_carpeta_compraventa IS
  'Založí klienta a desku compraventa. nombre + apellidos; NIE volitelné.';
