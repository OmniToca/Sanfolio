-- Chat panel zapisuje do ai_conversations / ai_messages.
-- 0018 mělo RLS, ale INSERT grant na hosted občas chybí: SELECT prázdného
-- vlákna projde, zápis zprávy spadne a asistent „nic nedělá“.
...
GRANT SELECT, INSERT, UPDATE ON TABLE public.ai_drafts TO authenticated;

-- Copilot musí vidět, že pas / NIE ve spisu je, i když gestor návrh ještě
-- neuložil. Prázdné extracted je platný stav, ne díra v RPC.
...
COMMENT ON FUNCTION public.ai_get_cliente(UUID) IS
  'Snapshot + díry + doklady (i bez uloženého extracted). Audit ai.read.cliente. Žádný save/send.';
