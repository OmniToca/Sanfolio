...
| `0070_staff_access.sql` | bez stropu 3 lidí; `staff_scopes` + `staff_cliente_access`; `can_access_cliente` / `staff_scope_set` |
| `0071_staff_access_surfaces.sql` | zápis + lhůty, pošta, faktury, storage, kampaně a AI v rozsahu člena |
| `0072_poder_glance_paper.sql` | čip poderu vrací papír (`documento_id`) k otevření |
| `0073_cliente_audit_staff.sql` | stopu karty čte člen s přístupem, ne jen owner |
| `0074_cliente_nombre_apellidos.sql` | `open_carpeta_compraventa` ukládá `nombre` + `apellidos`; karta má dvě pole |
| `0075_lock_is_support.sql` | C1: klient nesmí měnit `profiles.is_support` (trigger; jen service_role) |
| `0076_support_sessions_hardening.sql` | H2: sessions jen SELECT; mutate přes RPC; reason ≥ 12; `forbid_hard_delete` |
| `0077_rpc_cliente_scope.sql` | H1: DEFINER RPC + storage DELETE + staff/`documento_bloques` hard-delete → `can_access_cliente` |
| `0078_documento_ai_summary.sql` | `documentos.ai_summary` + `ai_summary_locale` (1–2 věty ve staff locale); anonymize maže |
| `0079_security_m5m7.sql` | M6: `impersonation_handoffs` (jednorázový code); M7: `similar_placed_papers` scoped = stejná karta |
| `0080_ai_assistant_search_fix.sql` | AI: `search_clients` jméno token+AND (ne `normalize_id`); `audit_logs` INSERT policy; `ai_get_cliente` + identifikátory |
| `0081_ai_search_free_text.sql` | AI/search: stopslova + NIE tokeny ve větě (`search_query_content` / `search_query_id_tokens`) |
| `0082_ai_nl_search_stoh.sql` | AI NL: soft jméno (skloňování), NIE substring, adresa bydliště+finca, RPC `search_cliente_documentos` (hromada) |

Edge: [`create-office`](../supabase/functions/create-office/index.ts) — založení kanceláře. [`translate-message`](../supabase/functions/translate-message/index.ts) — překlad výzvy (`tenant_id` + `can_access_tenant`; klíč `OPENAI_API_KEY`, jinak originál). [`impersonation-handoff`](../supabase/functions/impersonation-handoff/index.ts) — M6 create/redeem jednorázového kódu (raw refresh ne v URL). [`plazo-reminders`](../supabase/functions/plazo-reminders/index.ts) — ranní drafty, nikdy `sent` (tajný `CRON_SECRET` nebo service_role). [`invite-staff`](../supabase/functions/invite-staff/index.ts) — owner zve gestor/asistente (bez stropu počtu; rozsah karet v `staff_scopes`). [`ai-assistant`](../supabase/functions/ai-assistant/index.ts) — chat tools (search_clients + search_cliente_documentos hromada, get_cliente, query_suministro / plazos / escritura, search_document_text hybrid FTS+vektory), žádný zápis. [`extract-document`](../supabase/functions/extract-document/index.ts) — fotka/PDF → pending `ai_drafts`, LLM na pozadí; OCR `body_text` + kousky + `ai_summary` ve staff locale; album/tipo až Guardar. [`merge-document-pages`](../supabase/functions/merge-document-pages/index.ts) — fotky v pořadí → jedno PDF, zdroje soft-delete. [`embed-pending-chunks`](../supabase/functions/embed-pending-chunks/index.ts) — dopočet vektorů (exact service key / CRON_SECRET, ne forged JWT). [`sif-emit`](../supabase/functions/sif-emit/index.ts) — koncept vydané → Verifacti create; bez klíčů `sif_not_configured`; po 200 `pendiente`. [`sif-status`](../supabase/functions/sif-status/index.ts) — Ověřit u AEAT; `emitida` až přijme. Due diligence: [facturacion_verifactu.md](facturacion_verifactu.md). [`posta-inbound`](../supabase/functions/posta-inbound/index.ts) — kopie kancelářské schránky (tajný `POSTA_INBOUND_SECRET` / Svix ±5 min); ukládá metadata + HTML + přílohy, nepřiřazuje blok; bounce odchozí výzvy. [`send-client-message`](../supabase/functions/send-client-message/index.ts) — výzva přes Resend po kliknutí gestora; podpis z `tenant_settings`; bez `RESEND_API_KEY` Flutter otevře Gmail. CORS allowlist: `https://sanfolio.app` (+ www / `CORS_ALLOWED_ORIGINS` / `SUPPORT_APP_URL`).
...
První Support účet (**SQL editor / service_role**, ne z Flutter JWT — migrace `0075`):

```sql
-- Spouštět jako service_role / postgres v SQL editoru:
